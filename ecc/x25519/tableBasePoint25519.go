...
import Fp "github.com/cloudflare/circl/ecc/fp25519"

// SageMath script for generating the table.
//////////////////////////////////////////////
//  p = 2**255-19
//  Fp = GF(p)
//  sizePBytes = 32
//  N = 252
//  a2 = Fp(486662)
//  E = EllipticCurve(Fp,[0,a2,0,1,0])
//  G = E.lift_x(9)
//  G = -G if int(G[1])%2 == 0 else G
//  tableU = lambda n : [ (P[0]+1)/(P[0]-1) for P in [ 2**i*G  for i in range(n) ] ]
//  toBytesLittleEndian = lambda L : "},\n".join( [ "/* (2^%3i)P */ {"%i+",".join([ "0x%02x"%d  for d in  ZZ(ui).digits(2**8,None,sizePBytes) ]) for i,ui in enumerate(L) ] )+"}, "
//  print(toBytesLittleEndian(tableU(N)))
//////////////////////////////////////////////

// tableBasePoint25519 contains the set of points:
//    t[i] = (xi+1)/(xi-1),
// where (xi,yi) = 2^iG and G=(9,y) and y%2=1.
// Size = (252)*(256/8) = 8064 bytes.
var tableBasePoint25519 = [252]Fp.Element{
