...
func (h *Histogram) Shard(measurement uint64, nonce *prio3.Nonce, rand []byte,
...
func (h *Histogram) PrepSharesToPrep(prepShares []PrepShare) (*prio3.PrepMessage, error) {
	return h.p.PrepSharesToPrep(prepShares)
}

func (h *Histogram) PrepNext(state *PrepState, msg *prio3.PrepMessage) (*OutShare, error) {
	return h.p.PrepNext(state, msg)
}

func (h *Histogram) AggregationInit() AggShare { return h.p.AggregationInit() }

func (h *Histogram) AggregationUpdate(aggShare *AggShare, outShare *OutShare) {
	h.p.AggregationUpdate(aggShare, outShare)
}
...
func newFlpHistogram(length, chunkLen uint) *flpHistogram {
	h := new(flpHistogram)
	numCalls := (length + chunkLen - 1) / chunkLen
	h.length = length
	h.chunkLen = chunkLen
	h.Valid.MeasurementLen = length
	h.Valid.JointRandLen = numCalls
	h.Valid.OutputLen = length
	h.Valid.EvalOutputLen = 2
	h.Gadget = flp.GadgetParallelSum{Count: chunkLen}
	h.NumCalls = numCalls
	h.FLP.Eval = h.Eval
	return h
}

func (h *flpHistogram) Eval(out Vec, g flp.Gadget[poly, Vec, Fp, *Fp], numCalls uint, meas, jointRand Vec, shares uint8) {
	var invShares Fp
