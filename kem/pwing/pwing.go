// Package pwing implements the P-Wing PQ/T hybrid KEM
package pwing
...
const (
	...
	// Size of an P-Wing public key
	PublicKeySize = 1216 + 33

	// Size of an P-Wing private key
	PrivateKeySize = 32

	// Size of the seed passed to EncapsulateTo
	EncapsulationSeedSize = 64 + 16

	// Size of the established shared key
	SharedKeySize = 32

	// Size of an P-Wing ciphertext.
	CiphertextSize = 1120 + 33
)
...
func combiner(
	...
	_, _ = h.Write(ssm[:])
	_, _ = h.Write(ssx[:])
	_, _ = h.Write(ctx.Bytes())
	_, _ = h.Write(pkx.Bytes())

	_, _ = h.Write([]byte(`QSF-P256-MLKEM768-SHAKE256-SHA3256`))
...
func (pk *PublicKey) Pack(buf []byte) {
	...
	copy(buf[mlkem768.PublicKeySize:], pk.x.Bytes())
}
...
func deriveKeyPair(seed []byte, sk *PrivateKey, pk *PublicKey) {
	...
	sk.x, err = ecdh.P256().NewPrivateKey(seedx[:32])
	if err != nil {
		panic(err)
	}

	if !bytes.Equal(sk.x.Bytes(), seedx[:32]) {
		panic(fmt.Sprintf("generate key failed 1 %d %d\n%x\n%x", len(sk.x.Bytes()), len(seedx), sk.x.Bytes(), seedx))
	}

	pk.x = sk.x.Public().(*ecdh.PublicKey)
	sk.xpk = pk.x
}
...
// P-Wing standard, which is the reverse of the Circl KEM API.
//
...
func (pk *PublicKey) EncapsulateTo(ct, ss, seed []byte) {
	...
	ekx, err := ecdh.P256().NewPrivateKey(seedx[:32])
	if err != nil {
		panic(err)
	}

	if !bytes.Equal(ekx.Bytes(), seedx[:32]) {
		panic(fmt.Sprintf("generate key failed 2 %d %d\n%x\n%x", len(ekx.Bytes()), len(seedx), ekx.Bytes(), seedx))
	}

	ctx := ekx.Public().(*ecdh.PublicKey)

	ssx, err := ekx.ECDH(pk.x)
	if err != nil {
		panic(err)
	}
	...
	copy(ct[mlkem768.CiphertextSize:], ctx.Bytes())
}
...
func (sk *PrivateKey) DecapsulateTo(ss, ct []byte) {
	...
	ctx, err := ecdh.P256().NewPublicKey(ct[mlkem768.CiphertextSize:])
	if err != nil {
		panic(err)
	}
...
func (pk *PublicKey) Unpack(buf []byte) error {
	...
	pk.x, err = ecdh.P256().NewPublicKey(buf[mlkem768.PublicKeySize:])
	if err != nil {
		panic(err)
	}
